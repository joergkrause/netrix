...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:param name="mediaprefix" select="media" />
  ...
  <xsl:template match="a:blip">
    ...
    <xsl:element name="img">
      ...
      <xsl:attribute name="src">
        <xsl:value-of disable-output-escaping="yes" select="concat($mediaprefix,'/',substring-after(document('word/_rels/document.xml.rels')/rel:Relationships/rel:Relationship[@Id=$relid]/@Target,'media/'))" />
      </xsl:attribute>
    </xsl:element>
  </xsl:template>
