...
public class EntityTable
	{
		...
		public virtual Entity lookup(System.String name)
		{
			return (Entity) entityHashtable[name];
		}
		
		public virtual Entity install(System.String name, short code)
		{
			Entity ent = lookup(name);
			if (ent == null)
			{
				ent = new Entity(name, code);
				entityHashtable[name] = ent;
			}
			else
			{
				ent.code = code;
		...
		public virtual Entity install(Entity ent)
		{
			...
			tempObject = entityHashtable[ent.name];
			entityHashtable[ent.name] = ent;
			return (Entity) tempObject;
		}
		
		/* entity starting with "&" returns zero on error */
		public virtual short entityCode(System.String name)
		{
			int c;
			...
			if (name[1] == '#')
			{
				...
				try
				{
					if (name.Length >= 4 && name[2] == 'x')
					{
						...
						c = System.Convert.ToInt32(name.Substring(3), 16);
					}
					else if (name.Length >= 3)
					{
						c = System.Int32.Parse(name.Substring(2));
					}
				}
		...
		public virtual System.String entityName(short code)
		{
			System.String name = null;
			Entity ent;
			System.Collections.IEnumerator en = entityHashtable.Values.GetEnumerator();
			//UPGRADE_TODO: Method 'java.util.Enumeration.hasMoreElements' was converted to 'System.Collections.IEnumerator.MoveNext' which has a different behavior. "ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?index='!DefaultContextWindowIndex'&keyword='jlca1073_javautilEnumerationhasMoreElements'"
			while (en.MoveNext())
			{
				//UPGRADE_TODO: Method 'java.util.Enumeration.nextElement' was converted to 'System.Collections.IEnumerator.Current' which has a different behavior. "ms-help://MS.VSCC.v80/dv_commoner/local/redirect.htm?index='!DefaultContextWindowIndex'&keyword='jlca1073_javautilEnumerationnextElement'"
				ent = (Entity) en.Current;
				if (ent.code == code)
				{
					name = ent.name;
					break;
